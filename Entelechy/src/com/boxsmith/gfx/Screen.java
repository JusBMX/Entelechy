...
public class Screen {
	...
	private static final String FONT = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890 .,:-"; // Used for mapping in game text.
	...
	/**
	 * Renders text. Font sprites must be 7x8 pixels.
	...
	public void renderText(String message, int x, int y, boolean fixedToWorld) {
		for (int i = 0; i < message.length(); i++) {
			...
			if (charIndex >= 0) {
				renderSprite(x + (i * 7), y,
						new Sprite(8, 7, (charIndex % 18) * 7,(charIndex / 18) * 8, SpriteSheet.FONT),
						fixedToWorld);
			}
		}
	}
